...
// Reverses a sequence of four chars to an unsigned int
unsigned int endianReverse(unsigned char one, unsigned char two, unsigned char three, unsigned char four)
{
  unsigned int tmp = four;
  unsigned int tmp1 = three;
  unsigned int tmp2 = two;
  unsigned int ret;
  tmp *= 16777216;
  tmp1 *= 65536;
  tmp2 *= 256;
  ret = tmp + tmp1 + tmp2 + one;
  return ret;
}
...
void parseDirectory(int iDirOff, int iEntries, unsigned char buffer[])
{
    ...
    for(i = 0; i < (iEntries); i = i + 1)   {
		...
		//Display filename
		toDOSName(string, buffer, i*SIZE);
		printf("%-12s\t", string);
		
		//Display Attributes
		parseAttributes(string, buffer[i*SIZE+11]);
		printf("%s\t", string);
		
		//Display Time
		parseTime(string, endianSwap(buffer[i*SIZE+22],buffer[i*SIZE+23]));
		printf("%s\t", string);
		
		//Display Date
		parseDate(string, endianSwap(buffer[i*SIZE+24],buffer[i*SIZE+25]));
		printf("%s\t", string);
		
		//Display Size
		printf("%d\n", endianReverse(buffer[i*SIZE+28],buffer[i*SIZE+29],buffer[i*SIZE+30],buffer[i*SIZE+31]));
    }
